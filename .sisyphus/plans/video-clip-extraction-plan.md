...
| **Must have**   | P0   | Phase 6: Electron基础 | Electron+Vite框架、主进程/渲染进程架构 | v1.0    | 用户体验基础       |
| **Must have**   | P0   | Phase 6: 主窗口UI     | 拖拽上传、流程可视化、实时进度         | v1.0    | 核心交互界面       |
| **Must have**   | P0   | Phase 6: IPC集成      | 主进程与渲染进程通信、核心服务调用     | v1.0    | 架桥核心能力       |
...
#### 里程碑 1: MVP v0.1 内测版（Day 18）

**时间点**：Phase 0-5完成后（Day 1-14）+ 4天集成测试缓冲
...
| **代码质量**   | 单元测试覆盖率（整体） | >= 80%  | Vitest报告     |
| **代码质量**   | 单元测试覆盖率（核心模块） | >= 90%  | Vitest报告     |
